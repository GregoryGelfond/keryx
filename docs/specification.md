...
(Realised for the binary wire form as of Increment 4's reassembler (`Codec::reassemble`): a slot index over the reachable subgraph joins field atoms to their occupants; a managed stack reconstructs each `emit_<sort>` root's tree under the uniform nesting ceiling (99, the inbound constant) — no native recursion — top-down to discover and validate, then bottom-up children-first to build; terms raise back to scalars by the inverse §6 policy with term-type conformance checked; and the message serializes canonically on a keryx-sized thread, a map field's entries ordered by key after the engine's encode (which holds a map unordered; `codec::canonical`). One message per root, roots ordered by their marker's total symbol order; every message or every diagnosis, the diagnostic theory's `violates` atoms reported mode-free beside the reassembler's own refusals (the reassembler runs no solver, so it takes no mode; R4). An orphan field atom within an exported tree — its parent occupant descends from a marker root but its occupancy atom is absent — is refused (`ShapeViolation`), while an atom reachable from no marker is the model's private business (§12.1), ignored. The JSON and textproto encoders, the `.lp` read, and `keryx emit` complete Increment 4; set ordering by symbol order arrives with sets, Increment 5.)
...
- **Outbound (binary built at Increment 4; JSON, textproto, and the `.lp` read to come):** answer-set symbols — the consuming tool's solver's output, or `.lp` fixtures parsed through `raise` (Increment 4) — → a slot index over the reachable subgraph → validation (§12.3) → managed-stack reconstruction → the engine's validating setter → canonical serialization: the binary wire form built, JSON and textproto at Increment 4. One message per `emit_<sort>` root, deterministic; every message or every diagnosis (§6, property 4).
