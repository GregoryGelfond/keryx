...
(Realised for the binary wire form as of Increment 4's reassembler (`Codec::reassemble`): a slot index over the reachable subgraph joins field atoms to their occupants; a managed stack reconstructs each `emit_<sort>` root's tree under the uniform nesting ceiling (99, the inbound constant) — no native recursion — top-down to discover and validate, then bottom-up children-first to build; terms raise back to scalars by the inverse §6 policy with term-type conformance checked; and the message serializes canonically on a keryx-sized thread. One message per root, roots ordered by their marker's total symbol order; every message or every diagnosis, the diagnostic theory's `violates` atoms reported mode-free beside the reassembler's own refusals (the reassembler runs no solver, so it takes no mode; R4). An orphan field atom within an exported tree — its parent occupant descends from a marker root but its occupancy atom is absent — is refused (`ShapeViolation`), while an atom reachable from no marker is the model's private business (§12.1), ignored. The JSON and textproto encoders, the `.lp` read, and `keryx emit` complete Increment 4; set ordering by symbol order arrives with sets, Increment 5.)
