...
- **Open enums.** Editions expose openness as the resolved `enum_type` feature (proto3-era enums are open: unknown integers are legal on the wire). Policy for an unknown numeric value arriving inbound: the default is a **structured translation error** (loud, honest). `(keryx.unknown) = PRESERVE` opts into the escape hatch: an unknown wire number `N` maps to the term `unknown(N)`, carried on its field's atom exactly as a declared constant is. `unknown(N)` is then admitted to both the value sort and the membership table, one rule per enum-valued field `f` of the enum: `core.lp` gains `e(unknown(N)) :- f(_, unknown(N)).` (the value joins the sort) and `emit.lp` gains `ok_e(unknown(N)) :- f(_, unknown(N)).` (the value satisfies the membership obligation `:- s(P), f(P, V), reach(P), not ok_e(V).`), so a reachable field carrying `unknown(N)` serializes rather than being refused. Both rules bind `N` **only** through the field atom that carries it — a positive body literal over the `#defined` field predicate — never as a free variable, so grounding instantiates them over exactly the finite set of `unknown(N)` terms the answer set contains; a bare `e(unknown(N)).` or `ok_e(unknown(N)).` fact with an unbound `N` would range over clingo's integers and be unsafe. A non-`PRESERVE` enum gains neither rule, so any `unknown(N)` on it is refused outbound — UNSAT under the strict theory, a `violates` diagnostic under the diagnostic one (§12.2) — the same loud refusal the default gives inbound. Outbound, then, only declared constants (plus `unknown(N)` under `PRESERVE`) are serializable; anything else is a shape violation.
