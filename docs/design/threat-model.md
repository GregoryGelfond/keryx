...
**The panic-hook consequence, which this boundary must reconcile.** A panic hook runs at the panic site, *before* any `catch_unwind` unwinds. The CLI already installs one (`exit::contain`) whose notice reads "this is a bug in keryx." So a `DependencyFault` contained at a door would still trip that hook: the user would see "a bug in keryx" (false — the fault is upstream's, provoked by the input) *and* the `DependencyFault` diagnostic; under `--format json`, two arrays for one event. For a library consumer with the default hook, the contained panic still prints Rust's own `panicked at …`. The refinement's whole motivation is the user's experience, so the pass **owes** a reconciliation — mark the contained call so the hook stays quiet, neutralize the notice, or suppress the hook for that call, each with a cost — a tradeoff to adjudicate, not choose silently. It retires three standing statements of the old posture, all to be updated when it lands: architecture §6, the `descriptor::decode` note, and `docs/proto-support.md`.
...
- **Status.** Totality (`UncompilableSource` on a protox failure) and integrity/determinism ride the descriptor door's walk over the compiled set — *held at `b093008`*; the `DependencyFault` containment of a protox panic, and bounded depth (`collect_messages`) and bounded work, are *this pass*, as for that door; **confinement** is *unestablished* (below); **confidentiality** is *in question* (property 6).
- **Resource selection / confinement.** This door **selects files by design**: an `import` in adversary-supplied source names a file the resolver reads and folds into the set. So the general "nothing an adversary places in the input selects a resource" does **not** cover this door; its defended property is instead **confinement** — that the resolver reads only within the include roots, not a traversing (`..`), absolute, or symlinked import out of them. **Status: unestablished** — it depends on protox's `IncludeFileResolver`/`ChainFileResolver`, a fact to read from their source and then hold by an instrument (a traversing import, an absolute import, a symlinked root). Until then this door's confinement is *not committed as held*, and the recommendation is its mitigation.
...
| `.proto` source | files + roots | yes | — | filesystem; confinement unestablished; prefer descriptor sets |
...
- **protox confinement** — whether `IncludeFileResolver`/`ChainFileResolver` confine reads against traversing, absolute, and symlinked imports, and whether a crafted include can shadow the vendored registry or the well-known types (source door, confidentiality).
- **Source-import exfiltration** — whether files an adversary-directed import folds in reach the `.lp` output.
- **Option-admission integrity** — a crafted `keryx/options.proto` redefining an annotation (descriptor door).
- **Decode limits** — what prost-reflect's and protox's decode recursion limits are at the pinned versions, deciding the depth property's engine-side branch and where the deep-nesting instrument attaches.
