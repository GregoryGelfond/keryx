...
**Inbound (message → facts):** payload (binary today; JSON and textproto follow) → prost-reflect dynamic decode against the root type's descriptor, inside the dependency boundary's containment frame → the shredding walk, guided by the mapping model and a root constant: an explicit heap stack of message occupants under the uniform nesting ceiling (99 message-typed levels below the root — a door-admission policy, spec §8), presence decided from each field's totality (spec §5), every scalar and map key lowered by the §6 scalar policy, every enum number resolved against its mapping (spec §7.4) → `Facts`: the `Vec<Symbol>` of the library seam and the `.lp` fact module of the CLI seam, each fact built once as one `(predicate, arguments)` through `terms` — the head `Symbol` and the rendered statement from the same pair, so the two seams cannot disagree. Ground by construction (P10) — no grounder. Validation names field paths (a refusal is a `Diagnostic` at its field's fully-qualified proto path; an undecodable or over-deep payload at the whole-payload locus); partial shreds never delivered — every fact or every diagnosis.
