...
**Date:** 2026-08-31 (founding; revised through the inbound codec's binary format)
...
| R5 | **keryx parses no protobuf** — protox produces `FileDescriptorSet`, prost-reflect reads it | (affirms spec P9, §20) | `FileDescriptorSet` is the interface; the dynamic layer (prost-reflect) is mandatory because keryx's annotations are custom options that typed `prost` structs silently drop (§20, load-bearing). Symmetry: keryx parses nothing — protobuf via protox/prost-reflect, ASP text via themelios `raise`. (prost/prost-types decode the set once to read its `syntax` for editions before the engine — the decoded struct is discarded, so no typed struct feeds the schema, §18/§20.) One **bounded departure**, named: the source door's nesting guard scans a `.proto`'s lexical bracket structure — a total *measure* that builds no tree and assigns no meaning, not a parse — to pre-empt protox's unbounded parser; the threat model's property 3 records it. |
...
built for the binary wire format: `Codec`/`Facts` on the library seam, `keryx facts` on the CLI
seam (`.binpb` payloads), the §6 scalar policy, the managed-stack walk under the uniform nesting
ceiling, and the thermal example's `facts` half, golden; its JSON and textproto forms follow
within the increment. Outbound reassembly (Increment 4) and the annotation semantics with
