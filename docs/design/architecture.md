...
**Date:** 2026-08-31 (founding; revised through the inbound codec's binary and text formats)
...
built for the binary wire format and the protobuf text format: `Codec`/`Facts` on the library
seam, `keryx facts` on the CLI seam (`.binpb` and `.txtpb` payloads), the §6 scalar policy, the
managed-stack walk under the uniform nesting ceiling, the textproto pre-parse depth guard and
sized parse thread, and the thermal example's `facts` half, golden in both committed forms; its
JSON form follows within the increment. Outbound reassembly (Increment 4) and the annotation semantics with
